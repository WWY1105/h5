...
//js的控制器
    ...
    $(function () {
        if (shopId && version == "WXPAY" && basicName != "paymentCanvas" && basicName != "paymentee") {
            // 通过下面这个API隐藏右上角按钮
            var wxJson = {};
        ...
        $("body").on("click", ".coupon_show", function () {
            ...
            rest(uri, {}, "get", function (data) {
                if (data.code == 200) {
                    ...
                    $("#my-code").html("");
                    if (node.attr("id") && (result.category == "902" || result.category == "904" || result.category == "909")) {
        ...
        $("body").on('click', '.exchange', function () {
            if (!$(this).hasClass("disabled")) {
                ...
                var result = confirm('本次抢兑需要花费' + id[0] + '积分,确认抢兑?');
                if (result) {
                    ...
                    switch (payment) {
                        case "1005":
                            ...
                            pay.success = function () {
                                //todo alipay
                                rest("/order/" + order_id, {}, "get", function (data1) {
            ...
            if (data.result.needPhone) {
                ...
                if (j > 1) {
                    ...
                    if (data.result.point) {
                        ...
                        str = "<div class='v-reward'><div class='v-item'>" + data.result.reward + "元奖励金</div></div>"
...
function entrance() {
    ...
    if (key_json.t) {
        ...
        if (time > key_json.t) {
            location.href = "errorTimeOut.html";
            return;
        }
    }

    $.fn.cookie("t", "", {"expires": 0});
...
function selfPay() {
    app = new Vue({
        ...
        methods: {
            ...
            cancel: function (id) {
                var _self = this;
                var result = confirm("确认放弃后您需要重新出示给店员才可使用");
                if (result) {
    ...
    var app = new Vue({
            ...
            rest("/groupon/activity/" + key_json.aid, {}, "get", function (data) {
                if (data.code == 200) {
                    ...
                    function leftTimer() {
                        ...
                        Vue.set(_self.time, 1, checkTime(hours) + ":" + checkTime(minutes) + ":" + checkTime(seconds));
                    }
                    ...
                    if (version == "WXPAY") {
                        ...
                        rest("/auth/sign", wxJson, "post", function (data) {
                            if (data.code == 200) {
                                ...
                                wx.ready(function () {
                                    wx.hideMenuItems({
                                        menuList: ["menuItem:copyUrl", 'menuItem:share:timeline'] // 要隐藏的菜单项，只能隐藏“传播类”和“保护类”按钮，所有menu项见附录3
                                    });
                                    wx.onMenuShareAppMessage({
                                        ...
                                        imgUrl: _self.data.activity.picUrl, // 分享图标
        ...
        methods: {
            bargainFn: function () {
                rest("/groupon/activity/" + key_json.aid, {}, "post", function (data) {
                    if (data.code == 200) {
                        ...
                        setTimeout(function () {
                            ajaxUrl("groupon.html?v=1&aid=" + key_json.aid);
                        }, 3000);
                    } else {
                        alert(data.message);
                    }
            ...
            inviteFn: function () {
                ...
                var img1 = new Image();//bg
                img1.setAttribute('crossOrigin', 'anonymous');
                img1.src = location.origin + "/sui_assets/img/grounpon/BG2.png";
                var img3 = new Image();//avatar
                img3.setAttribute('crossOrigin', 'anonymous');
                img3.src = _self.data.avatarUrl;
                rest("/groupon/activity/" + key_json.aid + "/invitation", {}, "post", function (data) {
                    if (data.code == 200) {
                        ...
                        context.beginPath();
                        context.arc(155, 353, 75, 0, Math.PI * 2, true); //Math.PI*2是JS计算方法，是圆
                        context.closePath();
                        context.fill();

                        context.beginPath();
                        context.rect(107, 305, 98, 98);
                        context.fillStyle = 'white';
                        context.fill();

                        context.lineWidth = 1;
                        context.strokeStyle = '#7a2b30';
                        context.stroke();
                        /*------text-------*/
                        context.font = '18px Arial';
                        context.fillStyle = '#000000';
                        context.textAlign = 'center';
                        context.fillText(_self.data.nickname, 150, 160);

                        context.font = '18px Arial';
                        context.fillStyle = '#7a2b30';
                        context.textAlign = 'center';
                        context.fillText(_self.data.brandName + '很赞', 150, 195);
                        context.fillText('扫一扫帮我砍价哦', 150, 217);

                        context.font = '14px Arial';
                        context.fillStyle = '#2b529a';
                        context.fillText('最低' + _self.data.activity.finalAmount + '元买到' + _self.data.activity.benefits[0].name, 150, 250);
                        context.fillStyle = '#7a2b30';
                        if (_self.data.activity.unUsedCount) {
                            context.fillText('仅剩 ' + _self.data.activity.unUsedCount + ' 份', 150, 465);
                        }

                        //红色背景
                        context.drawImage(img1, 0, 0, 300, 140);

                        context.save(); // 保存当前ctx的状态
                        context.arc(152, 111, 22, 0, 2 * Math.PI);
                        context.clip(); //裁剪上面的圆形
                        if (_self.data.avatarUrl) {
                            context.drawImage(img3, 130, 88, 44, 44);
                        }
                        context.restore();

                        var img = new Image();//qrcode
                        var t = setInterval(function () {
                            if (qrcode._oDrawing._elImage.src) {
                                clearInterval(t);
                                img.src = qrcode._oDrawing._elImage.src;
                                setTimeout(function () {
                                    context.drawImage(img, 109, 308, 93, 93);
...
function mealDetail() {
    rest("/activities/activity/" + key_json.aid + "/setmeal/" + key_json.rid, {}, "get", function (data) {
        if (data.code == 200) {
            ...
            $("title").html(item.brandName);
            item.desUrl && $(".m-bg").attr("style", "background-image:url(" + item.desUrl + ")");
            $(".m-bg .m-title").html(item.name);
            item.remainCount && $(".relative-label").html("剩" + item.remainCount + "份");
            item.content && $(".m-text").html("<pre>" + item.content + "</pre>");
            $(".text-orange").html("￥" + item.benefit);
            $(".amount").html("￥" + item.amount);
            $(".set-content").html("");
            for (var i = 0; i < item.details.length; i++) {
                $(".set-content").append("<div>" + item.details[i].name + "<span class='count'>( " + item.details[i].count + "份）</span></div>");
            }
            item.logo && $(".avatar1").attr("src", item.logo);
            $("#loading").addClass("hide");
        }
    });
}
        ...
        if (data.code == 200) {
            ...
            for (var t = 0; t < cell.length; t++) {
                for (var r = 0; r < cell[t].coupons.length; r++) {
                    ...
                    if (coupon.category == "902" || coupon.category == "904") {
                    }
            ...
            if (!$(this).hasClass("disabled")) {
                ...
                rest("/phone/bindup", {
                    ...
                    if (data.code == 200) {
                        ...
                        $("#validate").val("");
                        clearInterval(init);
                        $(".input-text").removeClass("disabled").html("重新发送验证码");
                        $.toast(data.message);
                        $(this).removeClass("disabled");
                    }
                });
    ...
    rest("/order/guest/" + _id, {}, "get", function (data) {
        ...
        function setItems(items) {
            for (var i = 0; i < items.length; i++) {
                ...
                html.find("#finalAmount").html("实付款 ￥" + (items[i].amount - (items[i].reduceAmount || 0)));
...
function ruleFn() {
    Vue.component('rule', {
        props: ['data'],
        template: ' <div class="rule" v-if="data.lottery||data.activity"> <div class="title">活动详细规则</div>' +
        '<ul>' +
        '<li v-if="data.activity&&data.activity.rule">分享您对本店的品鉴赞赏给好友，即可邀请好友' +
        '<span v-if="data.activity.rule.participants">领取本店送出的优惠券，并</span>为您助力一票</li>' +
        '<li v-if="data.lottery">截止 {{data.lottery.endTime}}，系统将自动按照助力票数排名发放' +
        '<span class="half-line">特大奖励红包</span> ，届时将通过商家微信公众号或短信通知到中奖者' +
        ' <ul v-if="data.lottery" style="padding: 0 0 0 1rem;color: #f8e71c;font-size: .8rem;line-height:1.4">' +
        '<li v-for="item in data.lottery.prizes">好友助力排名第{{item.start}}<span v-if="item.end">~{{item.end}}</span>名，可获' +
        '{{item.value[0].name}}{{item.value[0].count}}张' +
        '</li>' +
        '</ul>' +
        '</li>' +
        '<li v-if="data.lottery">开奖时间：{{data.lottery.lotteryTime}}，请及时关注本店微信公众号，等待接收中奖信息。</li>' +
        '<li>最终解释权归本店所有</li>' +
        '</ul>' +
        '</div>'
    });
...
function payment() {
    ...
    var app = new Vue({
            ...
            rest("/praise/order/" + key_json.order, {}, "get", function (data) {
                if (data.code == 200) {
                    ...
                    if (data.result.winners) {
                        ajaxUrl("prize.html?comment=" + data.result.commentId);
                        return;
                    }
                    if (data.result.order) {
                        _self.currentView = "first";
                        if (data.result.activity) {
                            rest("/praise/dishes/" + key_json.id, {}, "get", function (data) {
                                if (data.code == 200) {
                                    _self.dishes = data.result;
                                    if (_self.dishes.pageSize > 1) {
                                        $(window).scroll(function () {
                                            if (_self.currentView != 'dish')return;
                                            var scrollTop = $(this).scrollTop();
                                            var scrollHeight = $(document).height();
                                            var windowHeight = $(this).height();
                                            if (scrollTop + windowHeight === scrollHeight) {
                                                _self.dishes.page++;
                                                rest("/praise/dishes/" + key_json.id, {page: _self.dishes.page}, "get", function (data) {
                                                    if (data.code == 200) {
                                                        _self.dishes.items = _self.dishes.items.concat(data.result.items);
                                                    }
                                                });
                                            }
                                        });
                            ...
                            if (!data.result.lastWinners) {
                                setTimeout(function () {
                                    $('#Marquee_x').jcMarquee({'marquee': 'x', 'margin_right': '100px', 'speed': 20});
                                }, 10);
                            }
                        }   //未绑定手机号
                        if ((data.result.order.obtained == false) && (data.result.order.strategy.coupons || data.result.order.strategy.points)) {
                            var str = "";
                            ...
                            $(".page-group").show().html('<div class="earn-true">' +
                            ...
                            $(".modal-button").click(function () {
                                $(".page-group").show().html('<div class="bg-green"><div style="padding-top: .5rem;font-size: .9rem">补全手机后，即可领取权益</div>' +
                                '<div class="text-center">' +
                                '<div style="height: 2px;background: #e1e1e1;margin: 1rem 40%"></div>' +
                                '<input type="tel" placeholder="您的常用手机号码" id="tel" maxlength="11"/>' +
                                '<div class="input-text">发送验证码</div>' +
                                '<input type="tel" placeholder="收到的验证码" id="validate" maxlength="6"/>' +
                                '</div>' +
                                '<div class="btn-green" id="submitFn">领取</div>' +
                                '<div class="md-close" onclick="$(\'.page-group\').hide();"></div></div>');
                                ...
                                $("#submitFn").off().click(function () {
                                    if ($("#tel").val().length == 11 && $("#validate").val().length == 6) {
                                        ...
                                        rest("/phone/bindup", jsonA, "post", function (result) {
                                            if (result.code == 200) {
                                                result.token && $.fn.cookie("token", result.token, {"expires": 7});
                                                $('.page-group').html("");
                                                $.toast("操作成功");
                                            } else {
                                                $("#validate").val("");
                    ...
                    } else if (data.result.qrCode) {
                        _self.currentView = "third";
                    } else if (!data.result.activity) {
                        _self.currentView = "third";
                    } else {
                        _self.currentView = "second";
                    }
                    _self.data = data.result;
                    ...
                    if (version == "WXPAY" && _self.data.commentId) {
                        var wxJson = {};
                        wxJson.url = location.href;
                        rest("/auth/sign", wxJson, "post", function (data) {
                            if (data.code == 200) {
                                ...
                                wx.ready(function () {
                                    ...
                                    wx.onMenuShareAppMessage({
                                        title: '我参加了' + _self.data.brandName + '的评赏', // 分享标题
                                        desc: '助力好友，领优惠', // 分享描述
                                        link: location.origin + '/paymentee.html?comment=' + _self.data.commentId, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
        ...
        methods: {
            couponShowFn: function (id) {
                couponShow(id)
            },
            joinFn: function () {
                var _self = this;
                this.post.comment = $("textarea").val();
                if (!this.post.comment || this.post.comment.length < 15) {
                    alert("请先填写15字以上评价");
                    return;
                }
                var dishArr = [];
                for (var i = 0; i < this.post.dishes.length; i++) {
                    if (this.post.dishes[i] && this.post.dishes[i].id) {
                        dishArr.push(this.post.dishes[i].id)
                    }
                }
                if (dishArr.length == 0) {
                    alert("请先选择满意的商品");
                ...
                rest("/praise/comment/" + key_json.order, {
                    ...
                    if (data.code == 200) {
                        ...
                        $(window).scrollTop(0);
                        //location.reload();
                        //ajaxUrl("payment2.html?comment=" + data.result.commentId)
                    } else {
                        alert(data.message);
                    }
                });
            ...
            commentFn: function () {
                rest("/praise/comment/" + this.data.commentId, {}, "get", function (data) {
                    if (data.code == 200) {
                        var str = "";
                        for (var i = 0; i < data.result.dishes.length; i++) {
                            str += '<div class="comment-img">' +
                            '<img src="' + data.result.dishes[i].picUrl + '"/>' +
                            '<div class="text">' + data.result.dishes[i].name + '</div></div>';
                        }
                        $(".page-group").html('<div class="bg-green1">' +
                        '<div class="box1">' +
                        '<div class="quote">' + data.result.comment + '</div>' +
                        '<div style="padding:1rem 0">' + str +
                        '</div></div></div>' +
                        '<div class="md-close" onclick="$(\'.page-group\').hide();"></div></div>');
                        $(".page-group").show();
                    } else {
            ...
            shareFn: function () {
                ajaxUrl("paymentCanvas.html?comment=" + this.data.commentId);
            },
            chooseImgFn: function (index) {
                var json = this.dishes.items[index];
                if (json.checked) {
                    this.count--;
                    json.checked = false;
                } else {
                ...
                Vue.set(this.dishes.items, index, json);

            },
            checkedImg: function () {
                this.post.dishes = [];
                for (var i = 0; i < this.dishes.items.length; i++) {
                    if (this.dishes.items[i].checked) {
                        this.post.dishes.push(this.dishes.items[i]);
                    }
                }
                for (var j = 0; j < 3; j++) {
                    if (this.post.dishes.length <= j) {
                        this.post.dishes.push({});
                    }
                }
...
function prize() {
    var app = new Vue({
        ...
        beforeCreate: function () {
            var _self = this;
            rest("/praise/winner/" + key_json.comment, {}, "get", function (data) {
                if (data.code == 200) {
                    $("#loading1").addClass("hide");
                    _self.data = data.result;
                }
        ...
        methods: {
            ...
            moreFn: function () {
                var _self = this;
                rest("/praise/winner/" + key_json.comment, {
                    page: this.data.winners.page + 1
                }, "get", function (data) {
                    if (data.code == 200) {
                        _self.data.winners.page++;
                        _self.data.winners.items = _self.data.winners.items.concat(data.result.winners.items);
                    }
                });
            }
        }
...
function couponShow(id) {
    ...
    rest(uri, {}, "get", function (data) {
        ...
        if (data.code == 200) {
            ...
            if (!html.length) {
                html = document.createElement("div");
                html.innerHTML = '<div class="popup popup-coupon bottom-fx"><div class="liner"><span class="text2"></span><span class="close" onclick="$(\'.page-group\').hide();"></span></div><div class="coupon-show-box"><div class="name_add"><div class="name"></div><div class="code"></div><div id="my-code"></div></div></div><div class="coupons-detail"></div>';
                $(".page-group").append(html);
                html = $(".popup-coupon");
            }
            html.find(".text2").html(categoty[result.category]);
...
function paymentCanvas() {
    var app = new Vue({
        ...
        beforeCreate: function () {
            var _self = this;
            rest("/praise/share/comment/" + key_json.comment, {}, "post", function (data) {
                if (data.code == 200) {
                    $("#loading1").addClass("hide");
                    _self.data = data.result;
                    _self.$nextTick(function () {
                        ...
                        html2canvas($("#htmlCode"), {
                            ...
                            onrendered: function (canvas) {
                                ...
                                newImg.src = dataUrl;
                                $("body").html('<div style="color: #ffffff;line-height: 3.5;font-size:36px">—— 将图展示给好友 长按可保存本图片 ——</div>');
...
function paymentee() {
    ...
    var app = new Vue({
        ...
        beforeCreate: function () {
            var _self = this;
            rest("/praise/share/comment/" + key_json.comment, {count: 1}, "get", function (data1) {
                if (data1.code == 200) {
                    if (data1.result.self) {
                        ajaxUrl("payment.html?order=" + data1.result.order.orderId);
                        return;
                    }
                    if (data1.result.candidaters || data1.result.benefits) {
                        _self.currentView = "second";
                    } else {
                        _self.currentView = "first";
                    }
        ...
        methods: {
            shareFn: function () {
                ajaxUrl("paymentCanvas.html?comment=" + key_json.comment);
            },
            couponShowFn: function (id) {
                couponShow(id)
            },
            submitFn: function () {
                //{phone:'',validateCode:''}无手机
                var _self = this;
                if (this.data.needPhone) {
                    ...
                    $("#submitFn").off().click(function () {
                        if ($("#tel").val().length == 11 && $("#validate").val().length == 6) {
                            $.showIndicator();
                            rest("/praise/upvote/" + key_json.comment, {
                                "phone": $("#tel").val(),
                                "validateCode": $("#validate").val()
                            }, "post", function (result) {
                                if (result.code == 200) {
                                    if (data.activity && data.activity.rule.participants) {
                                        $(".page-group").html('<div class="earn-true">' +
                                        '<div class="big-title">领取成功！</div>' +
                                        '<div>恭喜获得以下奖励</div>' +
                                        '<div class="coupon coupon-white" onclick="couponShowFn(' + data.activity.rule.participants[0].id + ')">' +
                                        '<div>' +
                                            //'<div style="font-size: .4rem">' + data.result.brand + '</div>' +
                                        '<div style="font-size: 1.2rem">' + data.activity.rule.participants[0].name + '</div>' +
                                        '</div>' +
                                        '<div class="detail"></div>' +
                                        '</div>' +
                                        '<div class="border">' +
                                        '<div>红包已发至您的账号</div>' +
                                        '<div>可在本店公众号中查看获赠权益</div>' +
                                        '</div>' +
                                        '</div>' +
                                        '<div class="md-close" onclick="$(\'.page-group\').hide();"></div>');
                                        $(".page-group").show();
                                    ...
                                    rest("/praise/share/comment/" + key_json.comment, {}, "get", function (data1) {
                                        if (data1.code == 200) {
                                            if (data1.result.self) {
                                                ajaxUrl("payment.html?order=" + data1.result.order.orderId);
                                                return;
                                            }
                                            if (data1.result.candidaters) {
                                                _self.currentView = "second";
                                            } else {
                                                _self.currentView = "first";
                                            }
                ...
                rest("/praise/upvote/" + key_json.comment, {}, "post", function (data) {
                    if (data.code == 200) {
                        $.toast("操作成功");
                        rest("/praise/share/comment/" + key_json.comment, {}, "get", function (data1) {
                            if (data1.code == 200) {
                                if (data1.result.candidaters || data1.result.upvoters) {
                                    _self.currentView = "second";
                                } else {
                                    _self.currentView = "first";
                                }
...
function birth() {
    var user = rest("/user", {}, "get", function (data) {
        ...
        $(".btn-orange").click(function () {
            if ($("input").val()) {
                var r = confirm("注意！请填写您的真实生日，生日一旦设置将不可更改！您设置的生日将可能影响您在商家正常行使权益。");
                if (r == true) {
                    var data = rest("/user", {
                        birthday: $("input").val()
                    }, "post", function (data) {
...
//充值记录
    ...
    $(document).on('infinite', ".infinite-scroll-bottom", function () {
        ...
        setTimeout(function () {
            ...
            page++;
            // 添加新条目
            var data = rest("/benefit/" + relationId + "/record/charge", {
                page: page,
                count: count
            }, "get", function (data) {
...
function phoneModal(a) {
    if (a == "phone") {
        $.modal({
            title: '<strong>【补全手机】</strong>',
            afterText: '<div class="text-xs" style="padding-top: .5rem">补全手机后，同步您已有权益/方便参与更多活动</div>' +
            '<div class="text-center">' +
            '<div style="height: 2px;background: #e1e1e1;margin: 1rem 40%"></div>' +
            '<input type="tel" placeholder="您的常用手机号码" id="tel" maxlength="11"/>' +
            ...
            '<input type="tel" placeholder="收到的验证码" id="validate" maxlength="6"/>' +
            '</div>' +
            '<div class="btn-green" style="margin: 1rem -1px 0;" id="bindPhone">确认</div>' +
            '<div class="md-close"></div>',
            extraClass: "bg-green"
        });
    } else {
        $.modal({
            title: '<strong>【补全手机 领取权益】</strong>',
    ...
    rest(url, json, "post", function (data) {
        switch (data.code) {
            ...
            case 405004:
                ...
                var r = confirm("您在" + (data.result.shopname || ("本店" + data.result.tableNo + "号桌")) + "有一个买单正在进行中,是否取消？");
                if (r == true) {
                    rest("/check/shop/" + data.result.shopId + "/cancel", {}, "post", function () {
                        if (data.code == 200) {
                            alert("取消成功！");
                            check();
                        }
                    });
                } else if (basicName == "waiting") {
                    wx.ready(function () {
                        wx.closeWindow();
                    })
                }
