...
//js的控制器
    ...
    $(function () {
        if (shopId && version == "WXPAY" && basicName != "praise" && basicName != "paymentee" && basicName != "comment") {
            // 通过下面这个API隐藏右上角按钮
            var wxJson = {};
...
function couponName(coupon) {
    switch (coupon.category) {
        case "901":
            str = "<div class='left'>￥<span style='font-size: 1.1rem'>" + coupon.amount + "</span></div><div class='right'>代金券</div>";
            break;
        case "902":
            str = "<div class='left'>￥<span style='font-size: 1.1rem'>" + coupon.currentAmount + "</span>" +
                "<div style='text-decoration: line-through'>" + coupon.amount + "</div>" +
                "<div class='right'>" + coupon.name + "</div>";
            break;
        case "908":
            str = "<div class='left'></div><div class='right' style='font-size: .9rem'>" + coupons.currentAmount + "元抵" + coupons.amount + "元</div>";
            break;
        case "904":
            str = "<div class='left'></div><div class='right' style='font-size: .9rem'>" + coupon.name + "</div>";
            break;
        case "9031":
            str = "<div class='left'></div><div class='right' style='font-size: .9rem'>" + coupon.name + "</div>";
            break;
            ...
            if (data.result.needPhone) {
                ...
                if (!str) {
                    ...
                    if (data.result.activities && data.result.activities.length) {
                        str += "<div style='max-height: 13rem;overflow: auto'><div style='text-align: left'><img src='/sui_assets/img/vipModal/ico3.svg'> 会员专属权益</div>";
                    ...
                    json.afterText = '<div class="upgrade-bg" style="background-image: url(\' ' + data.result.card + '\')"><img src="' + data.result.logo + '"></div>' + str;
                    json.buttons = [{
                        text: '立即领取',
                        bold: true,
                        onClick: function () {
                ...
                //}
                $.modal({
                    afterText: '<img src=\"' + data.result.picUrl + '\" style="height:9rem;width:100%">' +
...
function couponFn() {
    Vue.component('coupon', {
        props: ['item', 'start', 'end', 'use'],
        template: '<div class="item" v-bind:class="{\'used\':use}" v-on:click="couponDetailFn(item.id)">' +
        '<div class="left">' +
        '<div class="label" v-if="start">第{{start}}<span v-if="end">~{{end}}</span>名</div>' +
...
/*function couponFn() {
    Vue.component('coupon', {
        props: ['coupon', 'start', 'end'],
        template: '<div  v-on:click="couponDetailFn(coupon.id)" ><div class="cash-coupon" v-if="coupon.couponCategory==\'901\'">' +
        '<div class="bg" :style="{backgroundImage: \'url(\'+ (coupon.picUrl||\'\') +\')\'}">' +
        '<div class="left-label" v-if="start">第{{end?(start+\'~\'+end):start}}名</div>' +
        ...
        '</div>' +
        '</div>' +
        '<div class="object-coupon" v-else>' +
        '<div class="bg" :style="{backgroundImage: \'url(\'+ (coupon.picUrl||\'\') +\')\'}">' +
        '<div class="left-label" v-if="start">第{{end?(start+\'~\'+end):start}}名</div>' +
        '<div class="name">{{coupon.name}}' +
        '<div class="pull-right">￥{{coupon.amount}}</div>' +
        '</div>' +
        '</div>' +
        '<div class="text" v-if="coupon.descriptor">{{coupon.descriptor}}</div>' +
        '<div class="meal" v-if="coupon.couponCategory==\'9021\'">' +
        '<div class="meal-title">套餐菜品</div>' +
        '<div class="item" v-for="item in coupon.dishess">' +
        '{{item.name}}({{item.count}}份)' +
        '<div class="pull-right">￥{{item.price}}</div>' +
        '</div>' +
        '<div class="total">总价值￥{{coupon.total}}</div>' +
        '</div>' +
        ...
        beforeCreate: function () {
            ...
            rest("/praise/order/" + key_json.order, {}, "get", function (data) {
                if (data.code == 200) {
                    ...
                    if (data.result.step == 3) {
                        ajaxUrl("prize.html?cid=" + data.result.cadidateId)
                    }
                    $("#loading").addClass("hide");
                    _self.data = data.result;
                    //setTitle(data.result.brand + "领券助力");
                    if (!data.result.order.obtained && data.result.order.strategy && (data.result.order.strategy.coupons || data.result.order.strategy.points)) {
                        var str = "";
    ...
    var app = new Vue({
        ...
        beforeCreate: function () {
            ...
            rest("/praise/order/" + key_json.order, {}, "get", function (data) {
                if (data.code == 200) {
                    if (data.result.step < 3) {
                        ajaxUrl('praise.html?cid=' + data.result.commentId);
                        return;
                    }
                    if (data.result.step == 3) {
                        ajaxUrl("prize.html?cid=" + data.result.commentId)
                    }
                    $("#loading").addClass("hide");
                    _self.data = data.result;
                    //setTitle(data.result.brand + "领券助力");
...
function bindPhone(title, text) {
    ...
    $(".page-group").show().html('<div class="bg-green"><div class="title">' + (title || '补全手机 领取权益') + '</div>' +
        ...
        '<div class="close" onclick="  $(\'.page-group\').hide();$(\'#app\').css(\'overflow\', \'auto\');"></div></div>');
    ...
    var app = new Vue({
        ...
        methods: {
            ...
            joinFn: function () {
                ...
                if (!this.post.content || this.post.content.length < 5) {
                    $.toast("请先填写五个字以上评价");
                    return;
                }
    ...
    var app = new Vue({
        ...
        methods: {
            ...
            benefitFn: function () {
                ...
                rest("/praise/benefit", param, "get", function (data) {
                    if (data.code == 200) {
                        bindPhone("领取权益", data.result ? "验证手机号即获“ " + data.result[0].name + "”<br>成功后可在公众号查看" : "");
                        $("#submitFn").off().click(function () {
                            if ($("#tel").val().length == 11 && $("#validate").val().length == 6) {
                                $.showIndicator();
                                var jsonA = {};
...
function praise2() {
    ...
    var app = new Vue({
        ...
        beforeCreate: function () {
            var _self = this;
            rest("/praise/activity/comment/" + key_json.cid, {}, "get", function (data) {
                if (data.code == 200) {
                    $("#loading").addClass("hide");
                    _self.data = data.result;
                    if (data.result.lottery) {
                    ...
                    if (data.result.lastWinners) {
                        setTimeout(function () {
                            $('#Marquee_x').Marquee({'marquee': 'x', "margin_right": "10px", 'speed': 50});
                        }, 100);
                    }
                    if (version == "WXPAY") {
                        var wxJson = {};
                        ...
                        rest("/auth/sign", wxJson, "post", function (data) {
                                if (data.code == 200) {
                                    ...
                                    wx.ready(function () {
                                        ...
                                        wx.onMenuShareAppMessage({
                                            title: _self.data.lottery ? ('我正在参加' + _self.data.brand.brand + '的晒单评奖活动') : _self.data.nickname + "邀您领" + _self.data.brand.brand + '的红包', // 分享标题
                                            desc: '助力好友，领优惠', // 分享描述
                                            link: location.origin + "/paymentee.html?id=" + key_json.id + "&cid=" + key_json.cid, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
        ...
        methods: {
            inviteFn: function () {
                ...
                $(".gmadal-black").show();
                if ($("#htmlCode").html()) {
                    rest("/praise/share/comment/" + key_json.cid, {}, "post", function (data) {
                        if (data.code == 200) {
                            _self.data1 = data.result;
                            _self.$nextTick(function () {
                                var qrcode = new QRCode($("#my-code1"), {
                                    width: 100,
                                    height: 100
            ...
            pageFn: function () {
                var _self = this;
                rest("/praise/candidate/" + key_json.cid, {
    ...
    app = new Vue({
            ...
            beforeCreate: function () {
                ...
                rest("/praise/" + key_json.pid + "/activity/support", {}, "get", function (data) {
                    if (data.code == 200) {
                        if (data.result.step == 3) {
                            ajaxUrl("prize.html?cid=" + key_json.candidateId)
                        }
                        _self.data = data.result;
                            ...
                            rest("/auth/sign", wxJson, "post", function (data) {
                                if (data.code == 200) {
                                    ...
                                    wx.ready(function () {
                                        ...
                                        wx.onMenuShareAppMessage({
                                            title: _self.data.nickname + "邀您领" + _self.data.name + '的红包', // 分享标题
                                            desc: '助力好友，领优惠', // 分享描述
                                            link: location.href, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
                                            imgUrl: _self.data.picUrl, // 分享图标
                                            type: '', // 分享类型,music、video或link，不填默认为link
            ...
            methods: {
                    ...
                    rest("/praise/support/" + key_json.pid, this.post, "post", function (data) {
                        if (data.code == 200) {
                            ...
                            if (data.result && data.result.needPhone) {
                                ...
                                $("#app").css("overflow", "hidden");
                                $(".page-group").show().html('<div class="bg-green"><div class="title">' +
                                    '<img src="/sui_assets/img/payment1/success1.svg" style="vertical-align: middle;margin-right: 5px;">助力成功</div>' +
                                    '<div class="text">' + (data.result.benefit ? ("验证手机号即获“ " + data.result.benefit + "”<br>成功后可在公众号查看") : "") + '</div>' +
                                    '<input type="tel" placeholder="手机号" id="tel" maxlength="11"/>' +
                                    '<div class="input-text">获取验证码</div>' +
                                    '<input type="tel" placeholder="验证码" id="validate" maxlength="6"/>' +
                                ...
                                $("#submitFn").off().click(function () {
                                    if ($("#tel").val().length == 11 && $("#validate").val().length == 6) {
                                        ...
                                        jsonA.phone = $("#tel").val();
                                        jsonA.validateCode = $("#validate").val();
                                        jsonA.praiseId = data.result.praiseId;
                                        rest("/praise/benefit", jsonA, "post", function (data1) {
                                            if (data1.code == 200) {
                                                if (data.result.benefit) {
                                                    $.toast("红包已到账");
                ...
                participantFn: function () {
                    ...
                    /*
                                    rest("/praise/lottery/" + this.data.activityId, {
                                        "candidateId": this.data.candidateId,
                                        "guestId": this.data.brand.brandId
                                    }, "get", function (data) {
                                        if(data.code == 200){
                ...
                /*   submitFn: function () {
                       ...
                       if (this.data.needPhone) {
                           ...
                           $(".page-group").show().html('<div class="bg-green"><div style="padding-top: .5rem;font-size: .9rem"">补全手机后，即可领取权益</div>' +
                               ...
                               '</div>' +
                               '<div class="btn-green" id="submitFn">领取</div>' +
                               '<div class="md-close" onclick="  $(\'.page-group\').hide();$(\'#app\').css(\'overflow\', \'auto\');"></div></div>');
                           ...
                           $("#submitFn").off().click(function () {
                               if ($("#tel").val().length == 11 && $("#validate").val().length == 6) {
                                   $.showIndicator();
                                   rest("/praise/upvote/" + key_json.cid, {
                                       "phone": $("#tel").val(),
                                       "validateCode": $("#validate").val()
                                   }, "post", function (result) {
                                       if (result.code == 200) {
                                           if (_self.data.benefits) {
                                               $(".page-group").html('<div class="achieve">' +
                                                   '<div class="big-title">领取成功！</div>' +
                                                   '<div style="font-size: .7rem">恭喜获得</div>' +
                                                   '<div class="text-red" style="font-size: 1.2rem;padding: .7rem 0">' + (_self.data.benefits[0].couponCategory == '901' ? _self.data.benefits[0].amount + '元代金券' : _self.data.benefits[0].name) + '</div>' +
                                                   '<div class="border">' +
                                                   '<div>红包已发至您的账号</div>' +
                                                   '<div>可在本店公众号中查看获赠权益</div>' +
                                                   '</div>' +
                                                   '<div class="btn" onclick="refresh()">我知道了</div>' +
                                                   '</div>');
                                               $(".page-group").show();
                                           } else {
                                               $(".page-group").html('<div class="achieve">' +
                                                   '<div class="big-title">助力成功！</div>' +
                                                   '<div class="text-red" style="font-size: 1.2rem;padding: 1rem 0">恭喜助力成功</div>' +
                                                   '<div class="btn" onclick="refresh()">我知道了</div>' +
                                                   '</div>');
                                               $(".page-group").show();
                                           }
                           ...
                           rest("/praise/upvote/" + key_json.cid, {}, "post", function (data) {
                               if (data.code == 200) {
                                   $("#app").css("height", $(window).height() + "px");
                                   $("#app").css("overflow", "hidden");
                                   if (_self.data.benefits) {
...
function paymentee1() {
    ...
    app = new Vue({
        ...
        beforeCreate: function () {
            var _self = this;
            rest("/praise/share/comment/" + key_json.cid, {}, "get", function (data) {
                if (data.code == 200) {
                    if (data.result.self) {
                        if (data.result.step == 3) {
                            ajaxUrl("prize.html?cid=" + key_json.cid);
                        } else {
                            ajaxUrl("praise.html?cid=" + key_json.cid);
                        }
                        return;
                    }
                    if (data.result.step == 3) {
        ...
        methods: {
            ...
            submitFn: function () {
                ...
                if (this.data.needPhone) {
                    ...
                    rest("/praise/support/" + key_json.pid, {}, "post", function (data) {
                        if (data.code == 200) {
                            $("#app").css("height", $(window).height() + "px");
                            $("#app").css("overflow", "hidden");
                            if (_self.data.benefits) {
...
function prize() {
    ...
    var app = new Vue({
        ...
        beforeCreate: function () {
            ...
            rest("/praise/lottery/" + key_json.cid, {}, "get", function (data) {
                if (data.code == 200) {
                    $("#loading").addClass("hide");
                    _self.data = data.result;
                    // rest("/praise/winner/" + key_json.cid, {}, "get", function (data) {
                    //     if (data.code == 200) {
                    //         _self.winners = data.result;
                    //     }
                    // });
                }
            })
        ...
        methods: {
            ...
            pageFn: function () {
                var _self = this;
                rest("/praise/winner/" + key_json.cid, {
                    page: this.winners.page + 1
                }, "get", function (data) {
                    if (data.code == 200) {
                        _self.winners.page++;
                        _self.winners.items = _self.winners.items.concat(data.result.items);
                    }
                });
            },
            moreFn: function () {
                var _self = this;
                if (_self.upvoters.page >= _self.upvoters.pageSize) {
                    return;
                }
                rest("/praise/upvote/" + key_json.cid, {
                    page: ++_self.upvoters.page,
                    count: _self.upvoters.count
                }, "get", function (data) {
                    if (data.code == 200) {
                        // 添加新条目
                        _self.upvoters.items = _self.upvoters.items.concat(data.result.items);
                    }
                })
            }
        }
...
function paymentCanvas() {
    var app = new Vue({
        ...
        beforeCreate: function () {
            var _self = this;
            rest("/praise/lottery/comment/" + key_json.cid, {}, "post", function (data) {
                if (data.code == 200) {
                    _self.canvas = data.result;
                    if (data.result.service) {
                        _self.stars = {
